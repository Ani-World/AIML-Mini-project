...
def recommend_movies_by_cluster(movie_query, top_n=5):
    ...
    X_all = name_feat[feature_cols].fillna(0).values.astype(float)
    X_all_scaled = scaler.transform(X_all)
    name_feat["Cluster"] = kmeans.predict(X_all_scaled)

    cluster_id = name_feat.loc[idx, "Cluster"]
    same_cluster = name_feat[name_feat["Cluster"] == cluster_id].copy()
    same_cluster = same_cluster[same_cluster["Name"] != movie_name_exact]

    if same_cluster.empty:
        print("No other movies in this cluster.")
        return movie_name_exact, []

    recs = random.sample(same_cluster["Name"].tolist(), min(top_n, len(same_cluster)))

    return movie_name_exact, recs
